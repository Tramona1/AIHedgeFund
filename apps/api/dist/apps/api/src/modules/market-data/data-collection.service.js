...
export class DataCollectionService {
     ...
     */
    async collectStockQuote(symbol) {
        try {
     ...
     */
    async collectCompanyInfo(symbol) {
        try {
            ...
            // Extract values
            const { Name: name, Description: description, Sector: sector, Industry: industry, MarketCapitalization: marketCapStr, PERatio: peRatioStr, DividendYield: dividendYieldStr, EPS: epsStr, Beta: betaStr, "52WeekHigh": fiftyTwoWeekHighStr, "52WeekLow": fiftyTwoWeekLowStr, SharesOutstanding: sharesOutstandingStr } = companyData;
     ...
     */
    async collectBalanceSheet(symbol) {
        try {
            ...
            for (const report of data.annualReports) {
                const { fiscalDateEnding, reportedCurrency, totalAssets, totalCurrentAssets, cashAndCashEquivalentsAtCarryingValue, inventory, totalLiabilities, totalCurrentLiabilities, totalShareholderEquity, retainedEarnings, commonStock } = report;
                // Store in database
     ...
     */
    async collectRSI(symbol, timePeriod = 14) {
        try {
     ...
     */
    async collectAllDataForSymbol(symbol) {
        const results = {
