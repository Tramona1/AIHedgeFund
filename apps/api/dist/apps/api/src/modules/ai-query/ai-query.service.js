...
import { selectWhere, insertInto, safeEq, safeTable } from "../../lib/db-helpers.js";
...
const safeCompanyInfo = safeTable(companyInfo);
const safeAiQueries = safeTable(aiQueries);
...
export class AIQueryService {
                ...
                if (incomeData && incomeData.length > 0) {
                    ...
                    if (result) {
                        ...
                        for (const company of result) {
                            const matchingIncome = incomeData.find((inc) => inc.symbol === company.symbol);
                            if (matchingIncome) {
                        ...
                        if (parameters.revenueMin) {
                            result = result.filter((item) => item.totalRevenue >= parameters.revenueMin);
                        }
                        if (parameters.revenueMax) {
                            result = result.filter((item) => item.totalRevenue <= parameters.revenueMax);
                        }
                    }
                }
            }
    ...
    async getQueryHistory(userId, limit = 10, offset = 0) {
        try {
            const history = await selectWhere(aiQueries, safeEq(safeAiQueries.userId, userId));
    ...
    async getUserQueries(userId) {
        try {
            return await selectWhere(aiQueries, safeEq(safeAiQueries.userId, userId));
        }
        catch (error) {
