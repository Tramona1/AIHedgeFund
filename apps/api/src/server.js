...
function createFallbackMarketDataRouter() {
  ...
  // Log that we're using the fallback router
  logger.warn('Using fallback market data router. This should not be used in production.');
  
  // Import db and necessary schema
  let db;
  try {
    const dbModule = require('@repo/db');
    db = dbModule.db;
    logger.info('Successfully imported DB module for fallback router');
  } catch (error) {
    logger.error('Failed to import DB module for fallback router:', error.message);
  }

  // GET /api/market-data/stocks - Get stock data for requested symbols
  router.get('/stocks', async (req, res) => {
    try {
      // Parse symbol query parameters
      let symbols = [];
      ...
      // If we have DB access, try to query real data
      if (db) {
        try {
          const { stockData } = db._.schema;
          let result;
          
          if (symbols.length > 0) {
            // Query specific symbols
            const { inArray } = require('drizzle-orm');
            result = await db.select().from(stockData).where(inArray(stockData.symbol, symbols));
          } else {
            // Get all stocks with a limit
            result = await db.select().from(stockData).limit(100);
          }
          ...
          logger.error('Error querying database in fallback router:', dbError.message);
        }
      }
      
      // If we reach here, either there's no DB or the query failed
      // Return empty array instead of mock data
      logger.warn('Returning empty stock data in fallback route - no DB connection');
      return res.json({ 
        stocks: [],
        timestamp: new Date().toISOString(),
        note: 'No database connection available. Please check server configuration.'
      });
    } catch (error) {
      logger.error('Error in fallback stock data route:', error.message);
