...
def main():
    """
    Test script to manually trigger the daily newsletter generation.
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.info(f"Daily newsletter job completed successfully: {result}")
        else:
            logger.error(f"Daily newsletter API returned error: {response.status_code}")
            logger.error(f"Response: {response.text}")
                
    except Exception as e:
        logger.error(f"Error in daily newsletter job: {e}")
