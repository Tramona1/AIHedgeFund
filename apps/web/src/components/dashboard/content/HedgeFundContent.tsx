...
export function HedgeFundContent() {
  ...
  const [hedgeFundActivities, setHedgeFundActivities] = useState<StockUpdate[]>([])
  ...
  // Fetch hedge fund activities
  const fetchHedgeFundActivities = async () => {
    ...
    try {
      ...
      // Filter only hedge fund related updates
      ...
      setHedgeFundActivities(updates);
    } catch (error) {
      console.error("Error fetching hedge fund activities:", error);
      setError("Failed to load hedge fund activities. Please try again later.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  if (error) {
    return (
      <EmptyState
        title="Error loading hedge fund activities"
        description={error}
        icon={Briefcase}
        actionLabel="Try Again"
        onAction={fetchHedgeFundActivities}
      />
    );
  }

  if (!hedgeFundActivities.length) {
    return (
      <EmptyState
        title="No hedge fund activities"
        description={userState.tickers.length === 0 
          ? "Add tickers to your watchlist to see hedge fund activities"
          : "No recent hedge fund activities for your watched tickers"}
        icon={Briefcase}
      />
    );
  }
  ...
  return (
    <div className="space-y-4">
      {hedgeFundActivities.map((activity) => (
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <Badge 
                    variant={activity.eventType === 'hedge_fund_buy' ? 'default' : 'destructive'}
                    className="flex items-center gap-1"
                  >
                    {activity.eventType === 'hedge_fund_buy' ? (
              ...
              {activity.details && (
                <div className="mt-2 text-sm">
                  <div className="flex justify-between border-t pt-2">
                    <span>Fund: </span>
                    <span className="font-medium">{activity.details.fund || 'Unknown'}</span>
                  </div>
